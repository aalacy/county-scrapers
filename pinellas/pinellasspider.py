...
class Spider(threading.Thread):
    ...
    def _create_new_session(self):
        self.session = requests.Session()
        self.session.proxies = {'http': 'http://127.0.0.1:8121', 'https': 'http://127.0.0.1:8121'}
    ...
    def _write_error_download_ids(self, r):
        with open('error_duval_files.txt', 'a') as file:
            file.write(f'{r}\n')
        file.close()
    ...
    def _start(self, r):
        #40560488
        url = f'https://officialrecords.mypinellasclerk.org/Image/DocumentPdfAllPages/{r}'
        ua = get_random_ua()
        headers = {
            'User-Agent': ua,
        }
        self.response = self.session.get(url, headers=headers, verify=False)
    ...
    def upload_dump_to_s3(self, tmp_name, name):
        year = name.split('-')[0]
        bucketkey = 'pinellas/{}/{}'.format(year, name)
    ...
    def parse_html(self, r):
        url = f'https://officialrecords.mypinellasclerk.org/details/documentdetails/{r}'
        ...
        for i in html.xpath('//div[@class="docDetailRow"]'):
            ...
            if (key == 'Instrument' or key == 'Instrument Number'):
                instrument = value
            ...
            data[key] = value
        data['filename'] = f'{year}-{filename}_{r}.pdf'
        return data
...
# doc = "https://officialrecords.mypinellasclerk.org/details/documentdetails/15977317"
# pdf = "https://officialrecords.mypinellasclerk.org/Image/DocumentPdfAllPages/15977317"
