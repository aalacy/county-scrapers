...
client = boto3.client('s3', aws_access_key_id='AKIAT34F3EZ454WXW3NL', region_name='us-west-1', aws_secret_access_key='**Ctyn15kykdT')
sentry_sdk.init("http://54d29db6cbd44c38b639b3f4b5a6f833@sentry.codelab.pp.ua:9000/11", integrations=[AwsLambdaIntegration()])
...
def lambda_handler(event, context):
    ...
    if query:
        querydata = dict(map(lambda x: x.split('='), query.split('&')))
        qs = querydata.get('qs')
        r = querydata.get('r')
        s = querydata.get('s')
        nsc_name = querydata.get('nsc_n')
        nsc_value = querydata.get('nsc_v')
        cfn = querydata.get('cfn')
        t = querydata.get('t')
        if t != 'qwjf7Tds30KndbSi8fqprFs4mfaskjqppmSAA993':
            return {
                'statusCode': 401,
        ...
        cookies = {'ASP.NET_SessionId': s, nsc_name: nsc_value}
        ...
        if not valCfn:
            ...
            if info:
                ...
                if error_msg.startswith('There seems to be a problem accessing this document'):
                    return {
                        'statusCode': 404,
                        'headers': {'Content-Type': 'application/json',},
                        'body': json.dumps({'status': 'error', 'message': 'need update session'}),
                    }
                else:
                    return {
                        'statusCode': 404,
            ...
            return {
                'statusCode': 404,
                'headers': {'Content-Type': 'application/json',},
                'body': json.dumps({'status': 'error', 'message': 'missing cfn'}),
            }
        ...
        if not filename:
            return {
                'statusCode': 404,
                'headers': {'Content-Type': 'application/json',},
                'body': json.dumps({'status': 'error', 'message': 'file not finded'}),
            }
        ...
        for tr in html.xpath('//table')[0].xpath('./tr'):
            if len(tr.xpath('td')) == 8:
                ...
                data[t1.xpath('strong/text()')[0].replace('.', '').replace(':', '')] = t2.xpath('text()')[0] if t2.xpath('text()') else ''
                data[t3.xpath('strong/text()')[0].replace('.', '').replace(':', '')] = t4.xpath('text()')[0] if t4.xpath('text()') else ''
                data[t5.xpath('strong/text()')[0].replace('.', '').replace(':', '')] = t6.xpath('text()')[0] if t6.xpath('text()') else ''
                data[t7.xpath('strong/text()')[0].replace('.', '').replace(':', '')] = t8.xpath('text()')[0] if t8.xpath('text()') else ''
